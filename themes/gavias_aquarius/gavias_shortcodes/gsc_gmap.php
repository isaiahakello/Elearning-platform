...
if(!class_exists('gsc_gmap')):
   class gsc_gmap{
      ...
      public static function sc_gmap( $attr, $content = null ){
         ...
         print '<div class="gsc-gmap clearfix ' . $el_class . ' ' . $style . '"><div class="widget-inner">';
            if ( preg_match( '/^\<iframe/', $content ) ) {
               echo '<div class="map-content">' . $content . '</div>';
            } else {
               echo '<div class="map-content"><iframe width="100%" height="' . $size . '"  scrolling="no" marginheight="0" marginwidth="0" src="' . $content . '&amp;t=' . $type . '&amp;z=' . $zoom . '&amp;output=embed' . $bubble . '"></iframe></div>';
            }
            if($info){
               print '<div class="info-inner"><div class="info">' . $info . '</div></div></div>';
            }
         print '</div>';
      }
